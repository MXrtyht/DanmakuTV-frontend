...
import { onMounted, onUnmounted, ref, watch, defineProps, defineExpose } from 'vue';
...
const initWebSocket = () => {
    socket = new WebSocket(`ws://localhost:8084/wsserver/${props.token}`)
    ...
    socket.onmessage = (e) => {
        try {
            ...
            switch (response.type) {
                ...
                case 'danmaku':
                    if (response.data) {
                        let danmakuData = typeof response.data === 'string' 
...
const resetDanmakuState = () => {
    ...
    if (danmakuRef.value) {
        ...
        if (showDanmaku.value && props.isPlaying) {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
        const msg = {
            ...
            data: {
                ...
                videoTime: Math.floor(props.currentTime)
            },
            message: "发送弹幕",
...
watch(() => props.currentTime, (newTime) => {
    if (showDanmaku.value) {
        showDanmakuAtTime(Math.floor(newTime))
    }
})
...
watch(() => props.isPlaying, (isPlaying) => {
    ...
    if (isPlaying && showDanmaku.value) {
        danmakuRef.value.play()
    } else {
