  ...
  <div class="home">
    ...
    <div class="profile-header">
      ...
      <div class="profile-info">
        <el-avatar :size="80" class="user-avatar" :src="userInfo.avatar"> </el-avatar>
        <div class="user-name">
          {{ userInfo.name }}
        </div>
      </div>
...
const loadData = async () => {
  try {
    ...
    if (fanRes.data.code !== 200) {
      console.error('获取粉丝数和关注数失败:', fanRes.data.message)
      ElMessage.error('获取粉丝数和关注数失败')
      return
    }
    if (userInfoRes.data.code !== 200) {
      console.error('获取用户信息失败:', fanRes.data.message)
...
.header-background {
  ...
  filter: brightness(0.8); /* 可选：降低亮度使文字更清晰 */
}
...
.profile-info {
  ...
  display: flex;
  align-items: center;
  padding: 20px 0 20px 100px; /* 上 右 下 左 - 左边距60px */
  height: 100%;
...
.sticky-menu-container {
  ...
  box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); /* 滚动时添加阴影 */
}
